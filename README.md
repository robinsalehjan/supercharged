...
git clone --branch v2.2.0 --depth 1 git@github.com:robinsalehjan/supercharged.git
...
The shared MCP baseline is code-review-graph and OpenAI Developer Docs. Codex provides Axiom through its native plugin (including its skills and optional trusted hooks). Apple’s native `xcode` bridge is isolated behind `codex -p apple`; use `codex -p apple-headless` for XcodeBuildMCP outside an open Xcode project. The computer-use bridge remains disabled and optional; Playwright MCP is enabled. `codex -p review` layers xhigh reasoning over the base review tooling. `npm run check:mcps` completes the MCP initialize handshake against every enabled base server; pass `-- --profile apple` or `-- --profile apple-headless` to include that profile. Agent audits run the same probe, and managed-tool installation repairs CRG or XcodeBuildMCP when an otherwise current installation cannot initialize. `agent_config/managed_tools.json` exact-pins OpenWiki, Playwright CLI, Playwright MCP, Plannotator, code-review-graph, XcodeBuildMCP, and Obscura while recording tested compatibility floors for Codex, Claude, RTK, and Worktrunk. Axiom uses an immutable marketplace commit plus an expected plugin version; shared git skills also require commit SHAs. A weekly workflow proposes reviewed exact-pin updates. A second weekly workflow proposes asdf runtime pins within the lines declared in `agent_config/asdf_policy.json`; moving a line (a new Node.js LTS major, the Ruby 4.x line) or a held plugin such as Java stays manual.
