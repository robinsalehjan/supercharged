...
if [ -n "${PLAYWRIGHT_CLI_NPM_VERSION:-}" ]; then
    playwright_cli_version="$PLAYWRIGHT_CLI_NPM_VERSION"
else
    playwright_cli_version=$(npm view @playwright/cli version)
fi
if [ -n "${PLAYWRIGHT_MCP_NPM_VERSION:-}" ]; then
    playwright_mcp_version="$PLAYWRIGHT_MCP_NPM_VERSION"
else
