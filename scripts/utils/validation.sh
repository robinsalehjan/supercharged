...
extract_tool_version() {
    ...
    case "$cmd" in
        ...
        "playwright-cli"|"playwright-mcp")
            # The managed versions are checked through npm in the agent audit.
            echo "installed"
            ;;
        "xcodes")
        ...
        "obscura")
            ...
            ;;
        "simslim")
            # Homebrew presence is enough for validation; runtime behavior
            # depends on a selected Xcode and installed simulator runtime.
            echo "installed"
            ;;
        "periphery")
...
validate_installation() {
    ...
    validate_tool "playwright-cli" "" || ((warned++))  # Agent browser automation CLI
    validate_tool "playwright-mcp" "" || ((warned++))  # Agent browser automation MCP
